import { t as isFile } from "./fs.utils-D7Y_Ll2o.mjs";
import { n as getPackageScope, t as findProjectRoot } from "./project.utils-D1sIMVlp.mjs";
...
async function clean({ dryRun = false, verbose = false } = {}) {
	...
	try {
		for (const pattern of GLOB_DELETE_INCLUDE) {
			...
			const deletedPaths = await deleteAsync(finalPattern, {
				...
				onProgress: verbose ? (progress) => {
					...
					console.log(chalk[dryRun ? "gray" : "magenta"](`Progress: ${deletedCount}/${totalCount} (${percent.toFixed(1)}%)`));
				} : void 0,
		...
		if (totalFiles > 0) console.log(chalk.gray(`  ${totalFiles} total files ${dryRun ? "would be" : "were"} deleted\n`));
	} catch (error) {
...
//#endregion
export { GLOB_DELETE_INCLUDE, clean, clean as default };