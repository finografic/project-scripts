...
async function scheduleDeferredDeletion(itemPath, _relativePath) {
	try {
		if (process.platform === "win32") (0, node_child_process.spawn)("cmd", ["/c", `timeout /t 2 /nobreak && rmdir /s /q "${itemPath}"`], {
			detached: true,
			stdio: "ignore",
			shell: true
		}).unref();
		else (0, node_child_process.spawn)("sh", ["-c", `sleep 2 && rm -rf "${itemPath}" && find "$(dirname "${itemPath}")" -name "node_modules" -type d -empty -delete 2>/dev/null || true`], {
			detached: true,
			stdio: "ignore"
		}).unref();
		return true;
...
async function executeFromMemory(originalPath) {
	try {
		const tempDir = await node_fs_promises.default.mkdtemp(node_path.default.join((0, node_os.tmpdir)(), "purge-builds-"));
		const tempScript = node_path.default.join(tempDir, "purge-builds-detached.js");
...
`;
		await node_fs_promises.default.writeFile(tempScript, detachedScript);
		(0, node_child_process.spawn)(process.execPath, [tempScript], {
			detached: true,
			stdio: "ignore"
		}).unref();
		try {
			await node_fs_promises.default.access(originalPath);
		} catch {
			return true;
		}
		const spinner = (0, ora.default)("Waiting for node_modules deletion...").start();
		let attempts = 0;
		const maxAttempts = 10;
		while (attempts < maxAttempts) {
			await new Promise((resolve) => (0, node_timers.setTimeout)(resolve, 500));
			try {
				await node_fs_promises.default.access(originalPath);
			} catch {
...
function getCurrentExecutionPath() {
	...
	return node_path.default.resolve(scriptPath);
}
/**
...
function shouldDeferNodeModules(itemPath) {
	...
	return itemPath === node_path.default.join(workingDir, "node_modules") && currentScript.includes("node_modules");
}
/**
...
function shouldDelete(itemPath, itemName, isDirectory) {
	...
	const pathParts = itemPath.split(node_path.default.sep);
	if ([
		"apps",
		"packages",
		"config",
...
async function getDirectorySize(dirPath) {
	try {
		const items = await node_fs_promises.default.readdir(dirPath, { withFileTypes: true });
		let totalSize = 0;
		for (const item of items) {
			const itemPath = node_path.default.join(dirPath, item.name);
			if (item.isDirectory()) totalSize += await getDirectorySize(itemPath);
			else if (item.isFile()) {
				const stats = await node_fs_promises.default.stat(itemPath);
	...
	try {
		...
		for (const item of items) {
			...
			if (shouldDelete(itemPath, item.name, isDirectory)) {
				const size = isDirectory ? await getDirectorySize(itemPath) : (await node_fs_promises.default.stat(itemPath)).size;
				results.push({
					path: itemPath,
...
async function deleteItem(itemPath, isDirectory) {
	try {
		if (isDirectory) await node_fs_promises.default.rm(itemPath, {
			recursive: true,
			force: true
		});
		else await node_fs_promises.default.unlink(itemPath);
		return true;
	} catch {
		return false;
	}
	...
	try {
		...
		for (const item of items) if (item.isDirectory()) {
			...
			if (item.name === "node_modules") try {
				if ((await node_fs_promises.default.readdir(itemPath)).length === 0) emptyDirs.push(itemPath);
			} catch {}
			else {
...
async function purge({ dryRun = false, verbose = false, recursive = false, forceDetach = false } = {}) {
	...
	const workingDir = process.cwd();
	if (dryRun) {
		console.log(chalk.default.green("🔒 DRY RUN MODE - NO FILES WILL BE DELETED\n"));
		console.log(chalk.default.yellow("⚠️  This is a simulation only. Remove --dry-run to actually delete files.\n"));
	}
	console.log(chalk.default.gray(`Working Directory: ${workingDir}`));
	console.log(chalk.default.gray(`Mode: ${recursive ? "Recursive (deep)" : "Current level only"}`));
	console.log(chalk.default.gray(`Operation: ${dryRun ? "DRY RUN (simulation)" : "LIVE (actual deletion)"}`));
	const currentScript = getCurrentExecutionPath();
	console.log(chalk.default.gray(`Self-preservation: ${currentScript}\n`));
	const scanSpinner = (0, ora.default)("Scanning for build artifacts...").start();
	...
	console.log(chalk.default.gray(`   • ${dirCount} directories`));
	console.log(chalk.default.gray(`   • ${fileCount} files`));
	console.log(chalk.default.gray(`   • ${formatBytes(totalSize)} total size\n`));
	if (verbose || dryRun) {
		console.log(chalk.default.white("📝 Items to be processed:\n"));
		const directories = itemsToDelete.filter((item) => item.type === "directory");
		const files = itemsToDelete.filter((item) => item.type === "file");
		if (directories.length > 0) {
			console.log(chalk.default.cyan("📁 Directories:"));
			directories.forEach((item) => {
				const relativePath = node_path.default.relative(workingDir, item.path);
				console.log(chalk.default.gray(`   ${relativePath} (${formatBytes(item.size)})`));
			});
			console.log();
		}
		if (files.length > 0) {
			console.log(chalk.default.cyan("📄 Files:"));
			files.forEach((item) => {
				const relativePath = node_path.default.relative(workingDir, item.path);
				console.log(chalk.default.gray(`   ${relativePath} (${formatBytes(item.size)})`));
			});
	...
	if (dryRun) {
		console.log(chalk.default.yellow("🔒 DRY RUN: No files were actually deleted."));
		console.log(chalk.default.gray(`Would have freed ${formatBytes(totalSize)} of space.`));
		return;
	}
	...
	const deleteSpinner = (0, ora.default)("Deleting items...").start();
	let deletedCount = 0;
	let freedSpace = 0;
	let errorCount = 0;
	for (const item of immediateItems) {
		...
		if (await deleteItem(item.path, item.type === "directory")) {
			...
			errorCount++;
			if (verbose) console.log(chalk.default.red(`\nFailed to delete: ${relativePath}`));
		}
	}
	...
	if (deferredItems.length > 0) {
		...
		for (const item of deferredItems) {
			...
			let deleted = false;
			if (forceDetach) {
				console.log(chalk.default.cyan(`🧠 Attempting memory detachment for ${relativePath}...`));
				deleted = await executeFromMemory(item.path);
				if (deleted) console.log(chalk.default.green(`⏰ Memory-detached: ${relativePath} will be deleted after process exits`));
				else {
					console.log(chalk.default.cyan("⏰ Falling back to timer approach..."));
					deleted = await scheduleDeferredDeletion(item.path, relativePath);
					if (deleted) console.log(chalk.default.green(`⏰ Timer-scheduled: ${relativePath} will be deleted after process exits`));
				}
			} else {
				deleted = await scheduleDeferredDeletion(item.path, relativePath);
				if (deleted) console.log(chalk.default.green(`⏰ Scheduled: ${relativePath} will be deleted after process exits`));
			}
			if (!deleted) {
				console.log(chalk.default.yellow(`⏸️  Deferred: ${relativePath} (automatic deletion failed)`));
				console.log(chalk.default.gray(`   Run after this completes: rm -rf ${relativePath} && pnpm install`));
				console.log(chalk.default.gray("   Or try: pnpm clean --detach for automatic deletion"));
			}
		}
	}
	const cleanupSpinner = (0, ora.default)("Cleaning up empty directories...").start();
	...
	console.log(chalk.default.gray(`   • ${totalDeleted} items deleted`));
	console.log(chalk.default.gray(`   • ${formatBytes(totalFreed)} freed`));
	if (errorCount > 0) console.log(chalk.default.yellow(`   • ${errorCount} errors encountered`));
	console.log();
}
