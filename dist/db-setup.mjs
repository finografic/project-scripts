...
import { t as findScriptConfigFile } from "./config.utils-CLs1y3-0.mjs";
...
const loadSeedConfig = async ({ configFileGlob = PATH_FILES_CONFIG } = {}) => {
	...
	try {
		...
		if (error && typeof error === "object" && "code" in error && error.code === "ERR_UNKNOWN_FILE_EXTENSION") {
			...
			console.error(chalk.yellow("Node.js cannot import .ts files directly. Please run with: NODE_OPTIONS='--import tsx' db-setup"));
			console.error(chalk.yellow("TypeScript is required for type safety with SeedConfig/ViewConfig interfaces."));
			process.exit(1);
		}
...
if (![
	...
	"test",
	"production"
].includes(nodeEnv)) console.warn(chalk.yellow(`⚠️ Unexpected NODE_ENV: ${nodeEnv}, defaulting to development`));
...
//#endregion
export { PATH_FILES_CONFIG, PATH_FOLDER_SCHEMAS, SCHEMAS_BLOCKLIST, main as default };