...
async function scheduleDeferredDeletion(itemPath, _relativePath) {
	try {
		if (process.platform === "win32") spawn("cmd", ["/c", `timeout /t 2 /nobreak && rmdir /s /q "${itemPath}"`], {
			detached: true,
			stdio: "ignore",
			shell: true
		}).unref();
	...
	if (deferredItems.length > 0) {
		...
		for (const item of deferredItems) {
			...
			if (forceDetach) {
				...
				if (deleted) console.log(chalk.green(`⏰ Memory-detached: ${relativePath} will be deleted after process exits`));
				else {
					console.log(chalk.cyan("⏰ Falling back to timer approach..."));
					deleted = await scheduleDeferredDeletion(item.path, relativePath);
					if (deleted) console.log(chalk.green(`⏰ Timer-scheduled: ${relativePath} will be deleted after process exits`));
				}
			} else {
				deleted = await scheduleDeferredDeletion(item.path, relativePath);
				if (deleted) console.log(chalk.green(`⏰ Scheduled: ${relativePath} will be deleted after process exits`));
			}
			if (!deleted) {
