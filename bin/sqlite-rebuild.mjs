...
var SqliteRebuilder = class {
  ...
  logVerbose(message) {
    if (this.options.verbose) console.log(chalk.gray(`[DEBUG] ${message}`));
  }
  ...
  async scanPackages() {
    ...
    for (const packageDir of packageDirs) {
      ...
      try {
        ...
        const betterSqlite3Version = packageJson.dependencies?.['better-sqlite3']
  ...
  async checkVersionConsistency() {
    ...
    const versions = this.packages.filter((pkg) => pkg.hasBetterSqlite3).map((pkg) =>
      pkg.betterSqlite3Version
    );
  ...
  async rebuild() {
    ...
    try {
      ...
      if (!await this.testBetterSqlite3()) {
