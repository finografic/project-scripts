...
async function assertCleanWorkingTree() {
	...
	if (stdout.trim().length > 0) throw new Error("working tree is not clean\n→ commit or stash your changes before releasing");
}
async function assertOnReleaseBranch() {
	const { stdout } = await exec("git branch --show-current");
	const branch = stdout.trim();
	if (branch !== "master") throw new Error(`not on release branch (${DEFAULT_RELEASE_BRANCH})\n→ current branch: ${branch}`);
}
...
function parseReleaseType(argv) {
	const [, , type] = argv;
	if (type !== "patch" && type !== "minor" && type !== "major") throw new Error("invalid release type\n→ expected one of: patch | minor | major\n→ example: github-release patch");
	return type;
}
